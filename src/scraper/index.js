...
class Scraper {
    ...
    static async getAnilistInfo(title) {
        const response = await fetch('https://graphql.anilist.co', {
            ...
            body: JSON.stringify({
                query: `query { Media(search: "${title}", type: ANIME) { id title { romaji, english, native } coverImage { large, medium } isAdult format } }`
            })
        });
    ...
    static async getAnimes() {
        ...
        do {
            const $ = await this.getHtml(`https://anime-sama.fr/catalogue/?type[0]=Anime&page=${page}`);
            ...
            page++;
            console.log("page", page, "catalog.length", catalog.length, "hasNextPage", hasNextPage)
        } while (hasNextPage);
    ...
    static async getAnime(animeId) {
        const $ = await this.getHtml(`https://anime-sama.fr/catalogue/${animeId}/`);

        const title = $("#titreOeuvre").text();
        const synopsisHeader = $('h2').filter((i, el) => $(el).text().trim().toLowerCase() === 'synopsis');
    ...
    static async getSeasons(animeId) {
        ...
        // 1️⃣ Fusionner tout le texte (si plusieurs scripts)
        const code = scripts.join("\n");

        // 2️⃣ Extraire les appels à panneauAnime("nom", "url")
        const matches = [...code.matchAll(/panneauAnime\("([^"]+)",\s*"([^"]+)"\)/g)];

        // 3️⃣ Transformer en tableau d'objets
        const seasons = matches.map(([_, name, id]) => ({ name, id: id.split('/')[0] })).slice(1);
    ...
    static async getEpisodes(animeId, seasonId) {
        ...
        for (const lang of ["vf", "vostfr"]) {
            const url = `https://anime-sama.fr/catalogue/${animeId}/${seasonId}/${lang}/episodes.js`;
    ...
    static searchLocalAnimes(query, animes) {
        ...
        return animes.filter(anime => {
            ...
            const title = anime.title?.title?.romaji || anime.title?.title?.english || anime.title?.title?.native || anime.title || '';
    ...
    static loadAnimes() {
        try {
            const animesData = fs.readFileSync(path.join(__dirname, '..', 'data', 'animes.json'), 'utf8');
...
Scraper.getEpisodes("kaguya-sama-love-is-war", "saison1").then(console.log);